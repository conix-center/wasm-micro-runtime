...
static
bool aot_augment_globals(AOTCompData *comp_data, char** var_names, int num_vars) {
  uint32 i;
  AOTGlobal *globals;

  uint32 total_count = comp_data->global_count + num_vars;
  uint64 size = sizeof(AOTGlobal) * ((uint64) total_count);
  uint32 data_offset = comp_data->global_data_size;

  if (size >= UINT32_MAX || !(globals = wasm_runtime_realloc(comp_data->globals, (uint32)size))) {
      aot_set_last_error("allocate memory failed.");
      return false;
  }
